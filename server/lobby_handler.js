// Lobby and Room Management Socket Handlers

function handleCreateRoom(io, socket, rooms, getPublicRooms, data = {}) {
  const roomId = Math.random().toString(36).substring(2, 8).toUpperCase();
  const playerName = data.name || 'Player 1';
  const maxPlayers = data.maxPlayers || 4;
  const avatar = data.avatar || 0;
  const mode = data.mode || (data.special === true ? 'special' : 'classic');
...
function handleListRooms(socket, getPublicRooms) {
  socket.emit('publicRooms', getPublicRooms());
}

function handleUpdateRoomOptions(io, socket, rooms, getPublicRooms, data = {}) {
...
function handleJoinRoom(io, socket, rooms, getPublicRooms, data = {}) {
...
function handleReconnectSession(socket, rooms, data = {}) {
...
function handleAddBot(io, rooms, getPublicRooms, BOT_NAMES, data = {}) {
  if (!data.roomId) return;
  const room = rooms.get(data.roomId);
  if (!room || room.isPlaying || room.players.length >= room.maxPlayers) return;
...
function handleRemoveBot(io, rooms, getPublicRooms, data = {}) {
  if (!data.roomId) return;
  const room = rooms.get(data.roomId);
...
function handleQuickMatch(io, socket, rooms, getPublicRooms, data = {}) {
  let foundRoom = null;
...
function handleToggleReady(io, socket, rooms, data = {}) {
...
function handleLeaveRoom(io, socket, rooms, getPublicRooms, data = {}) {
  const roomId = data.roomId;
  const room = rooms.get(roomId);
  if (!room) return;
